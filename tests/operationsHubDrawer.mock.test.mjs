...
assert.match(app, /function renderDrawerInventoryChannel[\s\S]*?data-drawer-value="sellpia_current_stock"[\s\S]*?data-drawer-price-component="option"[\s\S]*?data-drawer-price-component="final"/, "drawer inventory must expose stock, option price, and final price inputs per seller");
