...
test('Sellpia patch exports selected columns and current formula values, without leaking inactive history',async()=>{
 const browser=await chromium.launch({channel:'msedge',headless:true});try{
  ...
  await page.setContent('<button id="sellpia-patch-open">patch</button>');await page.addStyleTag({path:'mockups/operations-hub/sellpia-patch-export.css'});await page.addScriptTag({path:process.env.XLSX_BROWSER_SCRIPT});await page.addScriptTag({path:'mockups/operations-hub/sellpia-patch-export.js'});
  await page.evaluate(()=>{
   ...
   SellpiaPatchExport.mount({rows:()=>fixture,selected:()=>chosen,loading:()=>false});
  });
  await page.locator('#sellpia-patch-open').click();assert.match(await page.locator('#sellpia-patch-preview').innerText(),/5566-1[\s\S]*59000/);assert.equal(await page.locator('#sellpia-patch-download').isDisabled(),false);
  ...
  await page.locator('[data-patch-field="purchase"]').check();assert.equal(await page.locator('#sellpia-patch-download').isDisabled(),true,'missing purchase price must not turn into zero');
  await page.evaluate(()=>{chosen=['5566-1'];});await page.locator('#sellpia-patch-scope').selectOption('selected');assert.match(await page.locator('#sellpia-patch-preview').innerText(),/53500/);assert.doesNotMatch(await page.locator('#sellpia-patch-preview').innerText(),/26750/);assert.equal(await page.locator('#sellpia-patch-download').isDisabled(),false);
  await page.evaluate(()=>{fixture[0].__hubInternalPrices.calculated_base_price.stale=true;});await page.locator('[data-patch-field="stock"]').uncheck();assert.equal(await page.locator('#sellpia-patch-download').isDisabled(),true);assert.match(await page.locator('#sellpia-patch-preview').innerText(),/재계산 필요/);assert.deepEqual(errors,[]);
...
test('real app toolbar connects search-type changes and Sellpia patch dialog',async()=>{
 const browser=await chromium.launch({channel:'msedge',headless:true});try{
  ...
  await page.locator('#sellpia-patch-open').click();assert.match(await page.locator('#sellpia-patch-preview').innerText(),/5566-1[\s\S]*59000/);assert.deepEqual(errors,[]);
 }finally{await browser.close();}
});
