...
assert.equal((html.match(/<th>판매가<\/th><th>옵션가<\/th><th>최종구매가<\/th>/g) || []).length, 3, 'every seller group must expose base, option, and final customer prices');
...
assert.match(csv, /판매가[\s\S]*?옵션가[\s\S]*?최종구매가/, 'matrix CSV must preserve the three user-visible price columns');
