...
function adapter(){
 ...
 const context={global:{crypto:{randomUUID:()=> 'request-id'}},cleanText:v=>String(v??'').trim(),requireOperationsHubSessionToken:()=> 'session',readableDatabaseError:e=>e,
  ...
  db:{async rpc(name,args){calls.push({name,args:structuredClone(args)});if(name==='hub_calculation_begin_v1')return {data:{generation_id:9,calculated_at:'now'},error:null};if(name==='hub_calculation_results_upsert_v1')return {data:{upserted:args.p_rows.length},error:null};
