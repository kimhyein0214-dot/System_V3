...
test('Sellpia CSV carrier preserves unrelated columns and blocks only the mismatched SKU',async()=>{
 ...
 try{
  ...
  const result=await page.evaluate(async()=>{
   ...
   const prepared=await SystemV3SellpiaCarrierExport.prepare([file],plan,'changed_only'),built=await SystemV3SellpiaCarrierExport.build(prepared),text=await built.blob.text(),output=XLSX.read(text,{type:'string',raw:true}),grid=XLSX.utils.sheet_to_json(output.Sheets[output.SheetNames[0]],{header:1,raw:true,defval:''});
   return {blocks:prepared.blocks,changed:prepared.changedSkuCount,grid};
  });
...
test('current three-file Sellpia CSV carrier stays bounded to one changed SKU',{skip:!process.env.SELLPIA_REAL_CARRIER_DIR},async()=>{
 ...
 try{
  ...
  const result=await page.evaluate(async()=>{
   ...
   const prepared=await SystemV3SellpiaCarrierExport.prepare(files,plan,'changed_only'),built=await SystemV3SellpiaCarrierExport.build(prepared),text=await built.blob.text(),output=XLSX.read(text,{type:'string',raw:true}),grid=XLSX.utils.sheet_to_json(output.Sheets[output.SheetNames[0]],{header:1,raw:true,defval:''});
