...
try{
 ...
 await page.locator('#rw-tag-import-file').setInputFiles({name:'공통 태그_일괄적용.csv',mimeType:'text/csv',buffer:Buffer.from('\ufeff셀피아 SKU\nsix\n')});await tagImportReady();await idle();
 await page.locator('#rw-tag-import-apply').click();await idle();assert.match(await page.locator('#rw-drawer-status').innerText(),/태그 DB 저장 완료/);assert.match(await page.locator('#rw-drawer-status').innerText(),/가격 재계산 실패\(태그 적용은 유지됨\)/);assert.equal(await page.evaluate(()=>qa.writes.filter(write=>write.action==='tag-excel-import').length),4,'formula refresh failure must not hide the completed tag DB write');
