...
assert.match(app, /tagApply[\s\S]*?saveSellerPriceDraft\([\s\S]*?targetBasePrice:calculatedBasePrice[\s\S]*?inputMode:'option'[\s\S]*?priceRuleSetId/, "applying a tagged sale price must preserve the option-price path through the atomic price RPC");
...
assert.match(app, /tagApply[\s\S]*?applyLocalSellerPriceDraft[\s\S]*?renderLiveMatrixRows\(matrixState\.rows\)[\s\S]*?renderDrawerInventory[\s\S]*?refreshChangeQueueInBackground/, "tagged price application must update the current matrix and drawer from local state without a blocking DB reload");
