...
begin
  ...
  loop
    ...
    if v_row.source_channel is not null and v_row.target_safety_state <> 'ready' then
      v_errors := v_errors || jsonb_build_array(
        case v_row.target_safety_state
          when 'conflict' then '연결된 구성 SKU의 수정값이 충돌합니다. 연결·세트 구성을 확인해주세요.'
          when 'incomplete' then '구성 SKU 또는 최신 판매처 원본의 재고를 확인할 수 없습니다.'
          else '내보내기 안전검사 상태를 확인할 수 없습니다.'
        end
      );
    end if;
