-- Read-only exact-SKU Matrix Grid v5 projection for affected-row patches.
-- The positional row SELECT is intentionally identical to the deployed v5
-- bootstrap feed; only its page selector is replaced with an indexed SKU set.
-- The active tag catalog and requested-SKU link badges accompany the rows so
-- normalization needs no full-dataset manifest refresh.

create or replace function public.hub_matrix_grid_rows_v5(
...
declare
  ...
  v_loaded integer;
  v_skus text[];
  v_missing_skus jsonb;
  v_tag_catalog jsonb;
  v_link_badges jsonb;
begin
  perform operations_private.require_operations_hub_operator_session(p_session_token);

  if p_skus is null or cardinality(p_skus) not between 1 and 200 then
    raise exception 'Matrix Grid targeted read SKU count must be 1~200.';
  end if;
  if exists (
    ...
    raise exception 'Matrix Grid targeted read contains invalid SKU.';
  end if;
  ...
  select coalesce(jsonb_object_agg(tag.tag_id::text, jsonb_strip_nulls(jsonb_build_object(
    'name', tag.tag_name, 'color', tag.tag_color, 'group', tag.tag_group
  ))), '{}'::jsonb)
    into v_tag_catalog
  ...
  select coalesce(jsonb_agg(jsonb_build_array(
    badge.sellpia_sku_code, badge.source_channel,
    badge.max_component_count, badge.relation_type
  ) order by badge.sellpia_sku_code, badge.source_channel), '[]'::jsonb)
    into v_link_badges
  from public.get_operations_hub_sku_link_badges_v2(v_skus) badge;

  select coalesce(jsonb_agg(requested.sku order by requested.sku), '[]'::jsonb)
    into v_missing_skus
  from unnest(v_skus) requested(sku)
  where not exists (
    select 1 from operations_private.operations_hub_matrix_export_cache cache
    where cache.sellpia_sku_code = requested.sku
  );
  ...
  seller_price_components as materialized (
    ...
    left join lateral (
      select source_row.base_price, source_row.price, source_row.discounted_base_price,
        source_row.option_price, source_row.final_price, source_row.discount_terms
      from public.seller_inventory_snapshot_rows source_row
      where source_row.snapshot_id = (
        select snapshot.snapshot_id
        from latest_seller_snapshot snapshot
        where snapshot.source_channel = mapped.source_channel
      )
...
revoke all on function public.hub_matrix_grid_rows_v5(text,text[]) from public;
revoke all on function public.hub_matrix_grid_rows_v5(text,text[]) from anon, authenticated;
grant execute on function public.hub_matrix_grid_rows_v5(text,text[]) to anon, authenticated, service_role;

comment on function public.hub_matrix_grid_rows_v5(text,text[]) is
  'Session-gated read-only exact-SKU v5 Grid rows, capped at 200 requested SKUs.';
