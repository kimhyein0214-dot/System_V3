...
begin
  ...
  if v_snapshot_id is null then
    raise exception '최신 판매처 원본 스냅샷이 없습니다.';
  end if;
  ...
  with base as (
    ...
    from public.operations_hub_matrix_managed_live m
    ...
    left join public.seller_inventory_snapshot_rows r
      on r.snapshot_id=v_snapshot_id
     and r.product_code=b.product_code
     and coalesce(r.option_code,'')=coalesce(b.option_code,'')
  ...
  select
    coalesce(jsonb_agg(jsonb_build_object(
      ...
      'source_discounted_base_price',source_discounted_base_price,
      'source_option_price',source_option_price,
      'source_final_price',source_final_price,
      'source_discount_terms',source_discount_terms,
    ...
    ) order by sellpia_sku_code),'[]'::jsonb),
    max(sellpia_sku_code),count(*)
