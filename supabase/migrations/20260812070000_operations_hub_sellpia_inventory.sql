...
select
  ...
  nullif(
    regexp_replace(coalesce(stock.raw_payload ->> 'sell_price', ''), '[^0-9.-]', '', 'g'),
    ''
  )::numeric as sellpia_sale_price,
...
comment on view public.operations_hub_matrix_live is
  'Read-only operations matrix enriched with the latest ready Sellpia stock snapshot. Seller inventory and price columns stay null until each seller snapshot adapter is connected.';
