-- Custom plan for the current bounded identities; values, guard clauses and output shape unchanged.
...
with batch as materialized (
 ...
 when $2='smartstore' then (select array_agg(c.sellpia_sku_code order by c.sellpia_sku_code) from operations_private.operations_hub_matrix_export_cache c where c.smartstore_product_code=scope_key::jsonb->>1)
 when $2='makeshop' then (select array_agg(c.sellpia_sku_code order by c.sellpia_sku_code) from operations_private.operations_hub_matrix_export_cache c where c.makeshop_product_code=scope_key::jsonb->>1)
 when $2='ably' then (select array_agg(c.sellpia_sku_code order by c.sellpia_sku_code) from operations_private.operations_hub_matrix_export_cache c where c.ably_product_code=scope_key::jsonb->>1)
 ...
 then operations_private.hub_input_scope_v1(seed,$2) else skus end skus from members
 )
 select jsonb_agg(jsonb_build_object('sku',x->>'sku','product_code',x->>'product_code','option_code',coalesce(x->>'option_code',''),
  ...
  'calculated',coalesce((select jsonb_agg(to_jsonb(cr)) from operations_private.hub_calculated_results cr where cr.sku=x->>'sku' and cr.scope in ('',$2)),'[]'::jsonb),
