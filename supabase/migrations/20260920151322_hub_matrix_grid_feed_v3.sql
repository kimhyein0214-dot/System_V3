-- Compact, session-gated Matrix bootstrap feed.
-- v3 keeps only Grid-visible/search/filter/sort state in the initial payload;
-- drawer/history/provenance details remain on the existing bounded readers.

create or replace function public.hub_matrix_grid_manifest_v3(
...
create or replace function public.hub_matrix_grid_feed_v3(
  p_session_token text,
  ...
  link_badges as materialized (
    select badge.sellpia_sku_code,
      jsonb_object_agg(badge.source_channel, jsonb_build_object(
        'max', badge.max_component_count,
        'relation', badge.relation_type
      )) payload
    ...
    select effective.sellpia_sku_code,
      jsonb_strip_nulls(
        jsonb_build_object(
          ...
          'sellpia_inventory_at', effective.sellpia_inventory_at,
          ...
          'sellpia_source_purchase_price', effective.sellpia_source_purchase_price,
          'sellpia_order_unit', coalesce(master.order_unit, effective.sellpia_source_order_unit),
          'sellpia_source_order_unit', effective.sellpia_source_order_unit,
          'sellpia_minimum_order_unit', coalesce(master.minimum_order_unit, effective.sellpia_source_minimum_order_unit),
          'sellpia_source_minimum_order_unit', effective.sellpia_source_minimum_order_unit,
        ...
        || jsonb_build_object(
          ...
          'smartstore_name_is_draft', case when effective.smartstore_suppressed then false else effective.smartstore_name_is_draft end,
          ...
          'makeshop_name_is_draft', case when effective.makeshop_suppressed then false else effective.makeshop_name_is_draft end,
          ...
          'ably_name_is_draft', case when effective.ably_suppressed then false else effective.ably_name_is_draft end,
        ...
        || jsonb_build_object(
          ...
          '__grid_compact', true,
          '__grid_meta', jsonb_strip_nulls(jsonb_build_object(
            'profile', jsonb_strip_nulls(jsonb_build_object(
              'product_code', effective.sellpia_product_code,
              ...
              'product_tag_ids', jsonb_path_query_array(effective.profile_json, '$.product_tags[*].tag_id'),
              'sku_tag_ids', jsonb_path_query_array(effective.profile_json, '$.sku_tags[*].tag_id')
            )),
            'drafts', active_drafts.payload,
            'link_drafts', product_link_drafts.payload,
            'link_badges', link_badges.payload,
            'seller_prices', seller_price_components.payload,
            'rule_prices', platform_prices.payload,
            'internal_prices', internal_prices.payload
          ))
        )
      ) row_json
    from effective
...
revoke all on function public.hub_matrix_grid_manifest_v3(text) from public;
revoke all on function public.hub_matrix_grid_manifest_v3(text) from anon, authenticated;
grant execute on function public.hub_matrix_grid_manifest_v3(text) to anon, authenticated, service_role;

revoke all on function public.hub_matrix_grid_feed_v3(text,timestamptz,text,integer) from public;
revoke all on function public.hub_matrix_grid_feed_v3(text,timestamptz,text,integer) from anon, authenticated;
grant execute on function public.hub_matrix_grid_feed_v3(text,timestamptz,text,integer) to anon, authenticated, service_role;

comment on function public.hub_matrix_grid_manifest_v3(text) is
  'Session-gated read-only Matrix v3 manifest with a deduplicated tag catalog.';
comment on function public.hub_matrix_grid_feed_v3(text,timestamptz,text,integer) is
  'Session-gated read-only compact Matrix v3 keyset feed. Returns only Grid-visible scalars and compact state.';
