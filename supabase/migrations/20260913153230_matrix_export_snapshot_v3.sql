...
begin
  ...
  with base as (
    select m.sellpia_sku_code,om.stock_quantity as system_stock,m.seller_drafts_json,
    ...
    from base b
    left join public.seller_inventory_snapshot_rows r on r.snapshot_id=v_snapshot_id and r.product_code=b.product_code and coalesce(r.option_code,'')=b.option_code
    left join operations_private.hub_calculated_results cr on cr.sku=b.sellpia_sku_code and cr.scope=v_source and cr.field='platform_registration_price'
    left join operations_private.hub_calculated_results cd on cd.sku=b.sellpia_sku_code and cd.scope=v_source and cd.field='platform_discount_price'
    left join operations_private.hub_calculated_results co on co.sku=b.sellpia_sku_code and co.scope=v_source and co.field='platform_option_price'
    left join operations_private.hub_calculated_results cf on cf.sku=b.sellpia_sku_code and cf.scope=v_source and cf.field='platform_final_price'
  )
  ...
  return jsonb_build_object('source',v_source,'snapshot_id',v_snapshot_id,'rows',v_rows,'next_cursor',v_next,'has_more',v_count=v_limit,'count',v_count);
end
$$;
