-- Carrier export inputs that are current facts/instructions only.
-- Historical materialized price results are deliberately excluded: the browser
-- evaluates the current Rule graph and uses that same effective tuple for the
-- Matrix and carrier export. Old generation failures remain audit data.
create function operations_private.carrier_effective_inputs_payload_v1(p_source text,p_skus text[])
...
create function public.hub_carrier_effective_inputs_read_v1(p_session_token text,p_source text,p_skus text[])
...
begin
 ...
 return operations_private.carrier_effective_inputs_payload_v1(p_source,p_skus);
end $$;
revoke all on function public.hub_carrier_effective_inputs_read_v1(text,text,text[]) from public;
grant execute on function public.hub_carrier_effective_inputs_read_v1(text,text,text[]) to anon,authenticated;
