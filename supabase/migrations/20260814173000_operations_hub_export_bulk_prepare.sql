create index if not exists operations_hub_change_queue_validated_source_idx
  on public.operations_hub_change_queue (source_channel, change_id)
...
begin
  if p_export_batch_id is null then
    raise exception '내보내기 배치 ID가 필요합니다.';
  end if;
  if p_change_ids is null or cardinality(p_change_ids) = 0 then
    raise exception '내보낼 변경사항을 선택해주세요.';
  end if;
  ...
  set status = 'failed',
      error_message = '30분 이상 완료되지 않아 자동 종료',
      updated_at = now()
  ...
  set status = 'failed',
      error_message = '원본 내보내기 작업 시간 초과',
      status_message = '내보내기 실패',
      updated_at = now()
  where queue.status = 'processing'
    and queue.last_attempt_at < now() - interval '30 minutes';

  insert into public.operations_hub_export_batches (
    export_batch_id, export_mode, source_channels
  ) values (
    p_export_batch_id, 'change_queue', v_sources
  );
  ...
  with source_specific as materialized (
    select
      ...
      queue.source_channel as export_source_channel,
      queue.field_key,
      queue.before_value,
      queue.after_value,
  ...
  global_changes as materialized (
    select
      queue.change_id,
      queue.sellpia_sku_code,
      source.source_channel as export_source_channel,
      queue.field_key,
      queue.before_value,
      queue.after_value,
  ...
  latest_snapshot as materialized (
    ...
    where snapshot.upload_status = 'ready'
      and snapshot.source_channel = any(v_sources)
  ...
  select
    p_export_batch_id,
    change.change_id,
    change.sellpia_sku_code,
    change.export_source_channel,
    change.field_key,
    change.before_value,
    change.after_value,
    change.product_code,
    coalesce(change.option_code, ''),
    source_row.raw_payload ->> 'source_file_name',
    source_row.source_row_no,
    case change.field_key
      when 'sellpia_current_stock' then to_jsonb(source_row.stock)
      when 'sellpia_sale_price' then to_jsonb(source_row.price)
    ...
    nullif(source_row.raw_payload ->> 'base_price', '')::numeric,
    nullif(source_row.raw_payload ->> 'option_price', '')::numeric,
    case
  ...
  left join latest_snapshot snapshot
    on snapshot.source_channel = change.export_source_channel
...
alter function public.complete_operations_hub_export(uuid,boolean,jsonb,text)
  set statement_timeout = '45s';
