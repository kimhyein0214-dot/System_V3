...
begin
  ...
  with base as (
    select
      m.sellpia_sku_code,
      om.stock_quantity as system_stock,
      m.seller_drafts_json,
    ...
    order by m.sellpia_sku_code
    limit v_limit
  ), enriched as (
    select
      b.*,
      r.source_row_no,
      r.raw_payload->>'source_file_name' as source_file_name,
      r.stock as source_stock,
      r.base_price as source_base_price,
      r.discounted_base_price as source_discounted_base_price,
      r.option_price as source_option_price,
      r.final_price as source_final_price,
      coalesce(r.discount_terms,'[]'::jsonb) as source_discount_terms,
      b.seller_drafts_json -> (v_source||':sellpia_current_stock') as stock_draft,
      b.seller_drafts_json -> (v_source||':sellpia_sale_price') as price_draft,
      cr.value as registration_price, cr.status as registration_status, cr.error as registration_error,
      cd.value as discount_price, cd.status as discount_status, cd.error as discount_error,
      co.value as option_price, co.status as option_status, co.error as option_error,
      cf.value as final_price, cf.status as final_status, cf.error as final_error,
  ...
  select
    coalesce(jsonb_agg(jsonb_build_object(
      'sku',sellpia_sku_code,'product_code',product_code,'option_code',option_code,
      'system_stock',system_stock,'seller_stock',seller_stock,'seller_price',seller_price,
      'source_row_no',source_row_no,'source_file_name',source_file_name,'source_stock',source_stock,
      'source_base_price',source_base_price,'source_discounted_base_price',source_discounted_base_price,
      'source_option_price',source_option_price,'source_final_price',source_final_price,
      'source_discount_terms',source_discount_terms,'stock_draft',stock_draft,'price_draft',price_draft,
      'registration_price',registration_price,'registration_status',registration_status,'registration_error',registration_error,
      'discount_price',discount_price,'discount_status',discount_status,'discount_error',discount_error,
      'option_price',option_price,'option_status',option_status,'option_error',option_error,
      'final_price',final_price,'final_status',final_status,'final_error',final_error,
