-- Old browser tabs used to request a full legacy-cache rebuild every minute.
-- Keep full rebuild authority server-side; public clients may only request a
-- bounded SKU refresh for backwards compatibility.
...
declare
  v_rows integer := 0;
begin
  if p_skus is null and current_user in ('anon', 'authenticated') then
    return 0;
  end if;
...
comment on function public.refresh_operations_hub_listing_legacy_cache(text[]) is
  'Refreshes legacy mapping edges; public browser roles cannot trigger an unbounded full-catalog rebuild.';
