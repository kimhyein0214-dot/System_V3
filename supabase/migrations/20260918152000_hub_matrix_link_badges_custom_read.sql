CREATE OR REPLACE FUNCTION public.get_operations_hub_matrix_link_badges_batch_v1(p_skus text[])
 RETURNS TABLE(sellpia_sku_code text, source_channel text, listing_count integer, max_component_count integer, relation_type text)
 LANGUAGE plpgsql
 STABLE
 SET search_path TO 'pg_catalog', 'public'
 SET jit TO 'off'
AS $function$ BEGIN
 IF p_skus IS NULL OR cardinality(p_skus) NOT BETWEEN 1 AND 200 THEN RAISE EXCEPTION 'invalid bounded Matrix badge request';END IF;
 RETURN QUERY EXECUTE $query$ with selected as materialized (
    select
    ...
    where component.is_active
      and component.sellpia_sku_code = any(coalesce($1, '{}'::text[]))
    ...
    where cache.sellpia_sku_code = any(coalesce($1, '{}'::text[]))
      and not exists (
        select 1
  ...
  group by selected.sellpia_sku_code, selected.source_channel $query$ USING p_skus;END $function$;
REVOKE ALL ON FUNCTION public.get_operations_hub_matrix_link_badges_batch_v1(text[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_operations_hub_matrix_link_badges_batch_v1(text[]) TO anon,authenticated,service_role;
...
begin
  ...
  EXECUTE $query$ SELECT jsonb_build_object(
    ...
    'link_badges', coalesce((
      ...
      from public.get_operations_hub_matrix_link_badges_batch_v1($1) badge
    ), '[]'::jsonb),
