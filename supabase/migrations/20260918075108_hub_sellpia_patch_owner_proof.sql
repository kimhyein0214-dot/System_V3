...
create or replace function operations_private.hub_sellpia_patch_read_v1(
...
begin
 ...
 'provenanceMismatch',exists(select 1 from operations_private.hub_rule_assignments a join operations_private.hub_rules ru on ru.id=a.rule_id and ru.is_active where a.sku=s and a.scope='' and a.target_field=r.field and not exists(select 1 from jsonb_array_elements(coalesce(r.rule_versions,'[]')) v where v->>'id'=ru.id::text and v->>'version'=ru.version::text and v->>'assignmentVersion'=a.version::text)),
...
create or replace function public.hub_sellpia_patch_read_v1(p_session_token text,p_skus text[] default null,p_search text default '',p_search_type text default 'sku',p_offset integer default 0,p_limit integer default 100,p_with_results boolean default true)
