alter function public.load_operations_hub_matrix_filtered(integer, integer, text, text[], text, text, jsonb, text[])
  rename to load_operations_hub_matrix_filtered_with_profiles;

create or replace function public.load_operations_hub_matrix_filtered_fast(
...
declare
  ...
  v_conditions jsonb := coalesce(p_filter -> 'conditions', '[]'::jsonb);
  v_logic text := lower(coalesce(p_filter ->> 'logic', 'and'));
  v_product_term text;
  v_option_term text;
  v_result jsonb;
begin
    ...
    where
      ...
      and (
        ...
        or (v_logic = 'and' and not exists (
          ...
          where not public.operations_hub_matrix_condition_matches(to_jsonb(matrix), '{}'::jsonb, condition)
        ))
        or (v_logic = 'or' and exists (
          select 1 from jsonb_array_elements(v_conditions) condition
          where public.operations_hub_matrix_condition_matches(to_jsonb(matrix), '{}'::jsonb, condition)
        ))
      )
  ), ranked as (
    select filtered.*,
...
revoke all on function public.load_operations_hub_matrix_filtered_fast(integer, integer, text, text[], text, text, jsonb, text[]) from public;
grant execute on function public.load_operations_hub_matrix_filtered_fast(integer, integer, text, text[], text, text, jsonb, text[]) to anon, authenticated;
...
begin
  ...
  if v_needs_profile then
    return public.load_operations_hub_matrix_filtered_with_profiles(p_page, p_page_size, p_search, p_search_sources, p_status, p_sort, v_filter, p_skus);
  end if;
  return public.load_operations_hub_matrix_filtered_fast(p_page, p_page_size, p_search, p_search_sources, p_status, p_sort, v_filter, p_skus);
end;
$$;

comment on function public.load_operations_hub_matrix_filtered(integer, integer, text, text[], text, text, jsonb, text[]) is
  'Routes common matrix-only filters through the fast path and attribute/tag filters through the profile-aware path.';
