...
begin
  ...
  if coalesce(array_length(p_skus, 1), 0) > 1000 then
    raise exception 'CSV 청크 SKU는 한 번에 최대 1000개까지 조회할 수 있습니다.';
  end if;
  ...
  with filtered as materialized (
    select matrix.*, to_jsonb(profile) as __profile
    ...
    select * from ranked
    where __row_number > v_offset
      and __row_number <= v_offset + v_limit
    order by __row_number
  )
  select jsonb_build_object(
    'rows', coalesce(
      jsonb_agg(
        (to_jsonb(paged) - '__row_number')
        || jsonb_build_object('__sellerDrafts', coalesce(drafts.payload, '{}'::jsonb))
        order by paged.__row_number
      ),
      '[]'::jsonb
    ),
    'offset', v_offset,
  ...
  left join lateral (
    ...
    where draft.sellpia_sku_code = paged.sellpia_sku_code
  ) drafts on true;
...
comment on function public.export_operations_hub_matrix_chunk(integer, integer, text, text[], text, text, jsonb, text[]) is
  'Exports one ordered matrix chunk with product profiles and active seller drafts for browser CSV generation.';
