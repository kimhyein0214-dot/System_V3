-- Keep advanced filtering on the established fast matrix source. The first
-- relation migration used the system overlay directly, which repeated stock
-- and master joins across the full catalog and approached statement timeout.
...
begin
  ...
  with filtered as materialized (
    ...
    left join public.operations_hub_product_profiles profile on profile.sellpia_sku_code = matrix.sellpia_sku_code
    where
      (
    ...
    select filtered.*,
      row_number() over (
        order by
          ...
          case when sellpia_sku_code ~ '^[0-9]+' then substring(sellpia_sku_code from '^([0-9]+)')::numeric else null end asc nulls last,
          (sellpia_sku_code ~ '^[0-9]+-[0-9]+') asc,
          case when sellpia_sku_code ~ '^[0-9]+-[0-9]+' then substring(sellpia_sku_code from '^[0-9]+-([0-9]+)')::numeric else null end asc nulls first,
          lower(sellpia_sku_code) asc
      ) as __row_number
    from filtered
  ), paged as (
    select * from ranked
    where __row_number > (v_page - 1) * v_page_size and __row_number <= v_page * v_page_size
...
comment on function public.load_operations_hub_matrix_filtered_v2(integer, integer, text, text[], text, text, jsonb, text[], boolean) is
  'Optimized advanced matrix filtering with explicit dependent-SKU exclusion and natural numeric SKU ordering.';
