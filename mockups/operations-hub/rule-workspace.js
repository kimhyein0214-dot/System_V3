(function(g){
 ...
 const fieldHelp={
  ...
  source_base_price:'셀피아 원본 파일의 판매가입니다. 시스템 기준가격과는 별개입니다.',
 ...
 const state={registry:{rules:[],assignments:[],dependencies:[]},tags:[],selected:null,steps:[],products:{},bulk:[],checks:new Set(),tab:'rules',busy:false,legacy:[],platformDoc:null,requested:[],tagContext:null,tagImport:null};
 ...
 legacyBackdrop.innerHTML=`<section class="rw-legacy-dialog" role="dialog" aria-modal="true" aria-labelledby="rw-legacy-title"><header class="rw-legacy-header"><div><h2 id="rw-legacy-title">기존 저장 수식 관리</h2><p>기존 계산 태그·할인 태그·가격 조합·실입고가 수식을 원래 편집 화면에서 관리합니다.</p></div><button class="btn" id="rw-legacy-close" type="button">닫기</button></header><p id="rw-legacy-status" class="rw-legacy-status" role="status"></p><div class="rw-legacy-body"></div></section>`;
 ...
 async function openLegacy(){
  ...
  legacyLoading=true;$('legacy-status').textContent='기존 저장 수식을 불러오는 중…';
  try{
   if(typeof g.SystemV3PriceRuleLab?.refresh!=='function'||typeof g.loadInboundCostTags!=='function')throw Error('기존 수식 편집 기능을 불러오지 못했습니다. 페이지를 새로고침해 주세요.');
   await Promise.all([g.SystemV3PriceRuleLab.refresh(),g.loadInboundCostTags({silent:false})]);legacyLoaded=true;
   $('legacy-status').textContent='기존 저장 수식은 그대로 유지됩니다. 이 화면은 기존 수식을 새 공통 수식으로 변환하지 않습니다.';
 ...
 function updateRuleScopes(){
  const platform=M.isPlatform($('target').value),sourcePlatform=M.isPlatform($('source-field').value),previous=$('scope').value;
  const choices=platform?{...(!current()?{'':'전체 판매처(스마트스토어·메이크샵·에이블리)'}:{}),...sources}:{'':'공통'};
  ...
  if($('source-scope-wrap'))$('source-scope-wrap').hidden=!sourcePlatform;
 ...
 function renderFormulaLibrary(){
  ...
  $('legacy-rules').innerHTML='<option value="">수식 태그 선택</option>'+(shared.length?`<optgroup label="수식 태그">${shared.map(rule=>`<option value="shared:${esc(rule.id)}">${esc(tagName(rule))} · ${esc(sources[rule.scope]||'공통')} ${esc(fieldLabels[rule.target_field]||rule.target_field)} · ${esc(operationSummary(rule))}</option>`).join('')}</optgroup>`:'')+(state.legacy.length?`<optgroup label="이전 형식 저장 수식">${state.legacy.map((entry,index)=>`<option value="legacy:${index}">${esc(entry.name)}</option>`).join('')}</optgroup>`:'');
  if(selected&&shared.some(rule=>`shared:${rule.id}`===selected))$('legacy-rules').value=selected;
 }
 async function loadLegacy(){const [registry,tags,price,inbound]=await Promise.all([D.ruleRegistry('list'),D.loadTags(),D.loadPriceRuleTags(),D.loadInboundCostFormulaTags()]);state.registry=registry;state.tags=tags||[];state.legacy=[...inbound.map(r=>({r,type:'inbound',name:r.tag_name})),...price.filter(r=>r.tag_role!=='discount').map(r=>({r,type:'price',name:r.tag_name}))];renderFormulaLibrary();status(`수식 태그 ${new Set(state.registry.rules.filter(rule=>rule.tag_id).map(rule=>rule.tag_id)).size}개와 이전 형식 수식 ${state.legacy.length}개를 불러왔습니다.`);}
 function importLegacy(){const value=$('legacy-rules').value;if(!value)return;readOps();if(value.startsWith('shared:')){const imported=state.registry.rules.find(rule=>rule.id===value.slice(7));if(!imported)return;state.steps=structuredClone(imported.config?.steps||[]);renderOps();updateRuleFlow();status(`${tagName(imported)}의 계산 단계 ${state.steps.length}개를 가져왔습니다. 현재 수식의 시작값과 저장 위치는 유지되며, 저장 전까지 DB는 변경되지 않습니다.`);return;}const entry=state.legacy[Number(value.replace('legacy:',''))];if(!entry)return;const f=g.TagPriceModel.fromSavedRule(entry.r,entry.type);state.importConfig={min:f.min,max:f.max};state.steps=[...(f.fixed!==''&&f.fixed!=null?[{op:'set',value:Number(f.fixed)}]:[]),{op:'multiply',value:f.multiplier},{op:'divide',value:f.divide||1},{op:'add',value:f.add},{op:'round',unit:f.unit,rounding:f.rounding}];renderOps();updateRuleFlow();status(`${entry.name}의 계산 단계를 가져왔습니다. 현재 수식의 시작값과 저장 위치는 유지되며, 저장 전까지 DB는 변경되지 않습니다.`);}
 ...
 function parseTagImportRows(sourceRows,{fileName=''}={}){
  const filenameTagName=tagNameFromFilename(fileName),normalized=value=>value.toLowerCase().replace(/[\s_-]+/g,'');
  if(filenameTagName){
   ...
   return {mode:'filename_tag',filenameTagName,rows:cells.map(sku=>({sku,tag_name:''}))};
  }
 ...
 function renderTagImport(){
  const data=state.tagImport||{mode:'single_tag',rows:[],server:null,localErrors:new Map()},server=data.server||{},serverRows=new Map((server.preview_rows||[]).map(row=>[row.row_no,row]));
  $('tag-import-mode').textContent=data.rows.length?(data.mode==='filename_tag'?`파일명 태그 자동 인식 · ${data.filenameTagName}`:data.mode==='single_tag'?'A열 SKU + 화면에서 공통 태그 선택':'A열 SKU + B열 태그명'):'파일을 선택하면 형식을 자동 판별합니다.';
  const fixedTagMode=data.mode==='per_row'||data.mode==='filename_tag';$('tag-import-tag').disabled=fixedTagMode;$('tag-import-tag-wrap').classList.toggle('rw-disabled',fixedTagMode);
  const localErrorCount=data.localErrors?.size||0,totalErrors=Number(server.error_count||0)+localErrorCount;
  $('tag-import-summary').textContent=data.rows.length?`전체 ${data.rows.length.toLocaleString('ko-KR')}행 · 적용 가능 ${Number(server.valid_count||0).toLocaleString('ko-KR')}행 · 중복 ${Number(server.duplicate_count||0).toLocaleString('ko-KR')}행 · 오류 ${totalErrors.toLocaleString('ko-KR')}행`:'아직 읽은 행이 없습니다.';
  $('tag-import-rows').innerHTML=data.rows.slice(0,200).map((row,index)=>{const checked=serverRows.get(index+1),resolvedTagId=data.mode==='filename_tag'?data.resolvedTagId:$('tag-import-tag').value,tag=checked?.tag_name||row.tag_name||(['single_tag','filename_tag'].includes(data.mode)?state.tags.find(item=>String(item.tag_id)===String(resolvedTagId))?.tag_name:''),error=data.localErrors?.get(index+1)||checked?.error||(checked?.is_duplicate?'중복 행 · 한 번만 적용':'정상');return `<tr><td>${index+1}</td><td>${esc(row.sku||'—')}</td><td>${esc(tag||'—')}</td><td class="${error==='정상'||error.startsWith('중복')?'':'rw-error'}">${esc(error)}</td></tr>`;}).join('')||'<tr><td colspan="4" class="rw-empty">XLSX, XLS 또는 CSV 파일을 선택하세요.</td></tr>';
 ...
 async function previewTagImport(sourceRows,{fileName=''}={}){
  ...
  if(parsed.mode==='filename_tag'){
   ...
   parsed.resolvedTagId=tag.tag_id;selectedTagId=tag.tag_id;
  }else if(parsed.mode==='single_tag')selectedTagId=$('tag-import-tag').value;
 ...
 g.HubPriceWorkspace={refresh:()=>run(refresh),openForTag,state,parseTagImportRows,tagNameFromFilename,openTagImport:options=>run(()=>openTagImport(options))};g.TagPriceWorkspace=g.HubPriceWorkspace;renderTab();
})(window);
