(function(g){
 ...
 const state={registry:{rules:[],assignments:[],dependencies:[]},selected:null,steps:[],products:{},bulk:[],checks:new Set(),tab:'rules',busy:false,legacy:[],platformDoc:null,requested:[],tagContext:null};
 ...
 function renderRules(){
  ...
  $('content').innerHTML=`<div class="rw-top"><aside class="rw-registry"><input id="rw-filter" placeholder="규칙 이름 검색" aria-label="규칙 이름 검색"><div id="rw-list"></div></aside><section class="rw-editor"><div class="rw-fields"><label>규칙 이름<input id="rw-name" value="${esc(r.name)}"></label><label>결과 저장 위치<select id="rw-target" aria-describedby="rw-stage-note">${option(Object.fromEntries(M.targets.map(k=>[k,fieldLabels[k]])),r.target_field)}</select></label><label>판매처<select id="rw-scope" aria-describedby="rw-stage-note">${option({'':'공통',...sources},r.scope)}</select></label></div><p id="rw-stage-note" class="rw-stage-note" hidden></p><div class="rw-source"><label>입력 위치<select id="rw-origin" aria-describedby="rw-stage-note">${option({self:'해당 SKU',parent:'상위 SKU'},r.input_origin)}</select></label><label>입력값 / 참조 필드<select id="rw-source-field">${option(fieldLabels,r.source_field)}</select></label><label>참조 판매처<select id="rw-source-scope">${option({'':'적용 판매처와 같음',...sources},r.source_scope||'')}</select></label></div><div class="rw-bar"><h3>계산 순서</h3><button class="btn" id="rw-add-op">+ 연산 추가</button></div><div class="rw-ops" id="rw-ops"></div><div class="rw-flow" aria-live="polite"><p id="rw-flow-summary"></p><p id="rw-flow-effect"></p></div><div class="rw-bar"><select id="rw-legacy-rules" aria-label="기존 저장 규칙"><option value="">기존 저장 규칙 불러오기</option></select><button class="btn" id="rw-legacy-load">목록 조회</button><button class="btn" id="rw-bulk">SKU 일괄적용</button><button class="btn primary" id="rw-save">${state.tagContext?'태그와 수식 저장':'규칙 저장'}</button></div><p id="rw-save-status" class="rw-save-status" role="status" aria-live="polite"></p></section></div><section class="rw-preview"><div class="rw-bar"><h3>적용 SKU · 저장값 미리보기</h3><input id="rw-preview-filter" placeholder="적용 SKU 검색 · 조회한 상품명" aria-label="적용 SKU 검색"><span id="rw-preview-count"></span></div><p id="rw-preview-state" class="rw-preview-state" role="status"></p><div class="rw-scroll"><table><thead><tr><th>SKU</th><th>상품 / 옵션</th><th>입력값</th><th>계산값</th><th>적용규칙</th><th>상태</th></tr></thead><tbody id="rw-preview"></tbody></table></div></section>`;
  renderList();renderOps();updateRuleScopes();$('target').onchange=updateRuleScopes;$('source-field').onchange=updateRuleScopes;$('filter').oninput=renderList;$('add-op').onclick=()=>{readOps();state.steps.push({op:'add',value:0});renderOps();};$('save').onclick=()=>run(saveRule);$('bulk').onclick=()=>run(openBulk);$('legacy-load').onclick=()=>run(loadLegacy);$('legacy-rules').onchange=importLegacy;$('preview-filter').oninput=()=>run(preview);
 ...
 function renderList(){const q=$('filter').value.toLowerCase(),counts=new Map();for(const a of state.registry.assignments)counts.set(a.rule_id,(counts.get(a.rule_id)||0)+1);$('list').innerHTML=state.registry.rules.filter(r=>(!state.tagContext||state.tagContext.id&&String(r.tag_id)===String(state.tagContext.id))&&r.name.toLowerCase().includes(q)).map(r=>`<button data-rule="${r.id}" aria-current="${r.id===state.selected}">${esc(r.name)}<small>${esc(fieldLabels[r.target_field])} ${esc(sources[r.scope]||'')} · ${counts.get(r.id)||0} SKU</small></button>`).join('')||'<p class="rw-empty">등록된 규칙이 없습니다.</p>';$('list').querySelectorAll('button').forEach(b=>b.onclick=()=>select(b.dataset.rule));}
 ...
 function importLegacy(){const entry=state.legacy[Number($('legacy-rules').value)];if(!entry)return;const f=g.TagPriceModel.fromSavedRule(entry.r,entry.type);state.selected=null;state.importConfig={min:f.min,max:f.max};$('name').value=entry.name;$('target').value=entry.type==='inbound'?'actual_inbound_cost':'calculated_base_price';$('origin').value='self';$('scope').value='';$('source-field').value=entry.type==='inbound'?'purchase_price':'actual_inbound_cost';updateRuleScopes();state.steps=[...(f.fixed!==''&&f.fixed!=null?[{op:'set',value:Number(f.fixed)}]:[]),{op:'multiply',value:f.multiplier},{op:'divide',value:f.divide||1},{op:'add',value:f.add},{op:'round',unit:f.unit,rounding:f.rounding}];renderOps();status('저장 규칙을 불러왔습니다. 새 공통 규칙으로 저장한 뒤 SKU에 연결하세요.');}
