(function(g){
 ...
 const fieldHelp={
  ...
  basis_sku_price:'다단계 계산 연결용 값입니다. 첫 Rule 결과를 다음 Rule의 시작값으로 이어줄 때만 사용합니다. 직접 값을 입력하는 항목이 아닙니다.',
  ...
  platform_price:'기존 규칙 호환용 필드입니다. 새 규칙에서는 등록가·옵션가·최종가를 직접 선택하는 편이 명확합니다.',
 ...
 function updateRuleFlow(){
  ...
  $('flow-effect').textContent=target==='calculated_base_price'?'통합 매트릭스에서 실제 기준가격으로 쓰는 값입니다. 일반적인 SKU 가격 수식은 보통 여기까지 계산하면 됩니다. 이후 판매처 가격은 각 판매처 규칙에서 이어서 계산합니다.':target==='basis_sku_price'?'다단계 계산 연결용 값입니다. 첫 Rule 결과를 여기에 저장한 뒤 다음 Rule에서 “이전 계산 결과”를 시작값으로 선택합니다. 직접 값을 입력하는 항목이 아닙니다.':target==='platform_discount_price'?'선택한 저장 판매처의 등록가에 할인만 적용한 단계입니다. 옵션가는 아직 반영되지 않습니다.':target==='platform_final_price'?'선택한 저장 판매처에서 할인과 옵션가까지 모두 반영한 최종 판매가입니다.':M.isPlatform(target)?'선택한 저장 판매처의 가격 단계에 계산 결과를 저장합니다.':'입력값에 계산 순서를 적용해 공통 시스템 값으로 저장합니다. 이 값을 참조하는 후속 규칙은 결과를 이어받을 수 있습니다.';
 ...
 function analyzeTagImport(rows,selectedTagId){
  ...
  rows.forEach((row,index)=>{
   ...
   for(const rule of state.registry.rules.filter(item=>String(item.tag_id)===String(tag.tag_id))){
    ...
    if(assigned&&assigned.rule_id!==rule.id){errors.set(index+1,`같은 계산 적용점에 이미 '${name(assigned.rule_id)}' Rule이 있습니다.`);break;}
    if(earlier&&earlier!==rule.id){errors.set(index+1,'이 파일 안에서 같은 계산 적용점의 서로 다른 Rule이 겹칩니다.');break;}
    if(rule.input_origin==='parent'&&!state.registry.dependencies.some(dep=>dep.child_sku===row.sku&&dep.rule_id===rule.id)){errors.set(index+1,'상위 SKU 입력 Rule은 종속관계를 먼저 등록해야 합니다.');break;}
 ...
 async function previewTagImport(sourceRows,{fileName=''}={}){
  ...
  renderTagImport();$('tag-import-file').onchange=()=>run(async()=>{const file=$('tag-import-file').files[0];await previewTagImport(await fileRows(file),{fileName:file.name});});$('tag-import-parse').onclick=()=>run(()=>previewTagImport(importTextRows($('tag-import-paste').value)));$('tag-import-tag').onchange=()=>{if(state.tagImport?.rows.length&&state.tagImport.mode==='single_tag')void run(()=>previewTagImport(state.tagImport.rows.map(row=>[row.sku])));};$('tag-import-apply').onclick=()=>run(applyTagImport);
 }
