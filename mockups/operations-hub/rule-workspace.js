(function(g){
 ...
 function parseTagImportRows(sourceRows){
 ...
 function renderTagImport(){
  ...
  $('tag-import-mode').textContent=data.rows.length?(data.mode==='single_tag'?'A열 SKU + 화면에서 공통 태그 선택':'A열 SKU + B열 태그명'):'파일을 선택하면 형식을 자동 판별합니다.';
  $('tag-import-tag').disabled=data.mode==='per_row';$('tag-import-tag-wrap').classList.toggle('rw-disabled',data.mode==='per_row');
  ...
  $('tag-import-rows').innerHTML=data.rows.slice(0,200).map((row,index)=>{const checked=serverRows.get(index+1),tag=checked?.tag_name||row.tag_name||(data.mode==='single_tag'?state.tags.find(item=>String(item.tag_id)===String($('tag-import-tag').value))?.tag_name:''),error=data.localErrors?.get(index+1)||checked?.error||(checked?.is_duplicate?'중복 행 · 한 번만 적용':'정상');return `<tr><td>${index+1}</td><td>${esc(row.sku||'—')}</td><td>${esc(tag||'—')}</td><td class="${error==='정상'||error.startsWith('중복')?'':'rw-error'}">${esc(error)}</td></tr>`;}).join('')||'<tr><td colspan="4" class="rw-empty">XLSX, XLS 또는 CSV 파일을 선택하세요.</td></tr>';
  $('tag-import-apply').disabled=!data.rows.length||!data.server||totalErrors>0||Number(server.valid_count||0)<1;
 }
 async function previewTagImport(sourceRows){
  const parsed=parseTagImportRows(sourceRows);if(!parsed.rows.length)throw Error('등록할 SKU 행이 없습니다.');if(parsed.rows.length>50000)throw Error('한 번에 최대 50,000행까지 등록할 수 있습니다.');
  state.tagImport={...parsed,server:null,localErrors:new Map()};renderTagImport();const selectedTagId=parsed.mode==='single_tag'?$('tag-import-tag').value:null;
 ...
 g.HubPriceWorkspace={refresh:()=>run(refresh),openForTag,state,parseTagImportRows};g.TagPriceWorkspace=g.HubPriceWorkspace;renderTab();
})(window);
