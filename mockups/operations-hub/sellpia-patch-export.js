(function(root){
 ...
 function mount(host=document.querySelector('#export-workflow-v2 .export-channel-grid')){
  ...
  function invalidate(message='대상 변경 · 원본 carrier 미리보기를 다시 실행하세요.'){rows=[];loadedScope=[];plan=null;prepared=null;output=null;carrierSnapshotId='';$('download').disabled=true;$('blocked-download').disabled=true;$('preview').innerHTML='';$('page').textContent='';status(message);}
  function render(){
   if(!plan)return;
  ...
  async function revalidate(){
   ...
   const carrier=await root.SystemV3Data.loadLatestSellpiaOriginalStatus();if(carrier.snapshotId!==carrierSnapshotId){invalidate();throw Error('최신 Sellpia 원본 carrier가 변경됐습니다. 미리보기를 다시 실행하세요.');}
  }
